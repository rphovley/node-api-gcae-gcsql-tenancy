...
export class EndpointController {
  ...
  public static async create(req: Request, res: Response, next: NextFunction): Promise<void | NextFunction> {
    ...
    try {
      const endpoint = await Endpoint.query().insertAndFetch(body)
      let params = []
      let query_params = []
  ...
  public static async update(req: Request, res: Response, next: NextFunction): Promise<void | NextFunction> {
    const { body } = req
    const endpointId = req.params.id
    if (!endpointId) return next({ status: 422, message: 'id required to update endpoint' })
    try {
      const endpoint = await Endpoint.query().patchAndFetchById(endpointId, body)
  ...
  public static async delete(req: Request, res: Response, next: NextFunction): Promise<void | NextFunction> {
    const endpointId = req.params.id
    if (!endpointId) return next({ status: 422, message: 'id required to update endpoint' })
    try {
      const endpoint = await Endpoint.query().deleteById(endpointId)
